      ...
      <header className="bg-neutral-900 border border-neutral-800 rounded-[32px] p-3">
        <div className="flex flex-col md:flex-row p-3 md:p-16 space-y-6 space-y-reverse md:space-y-0 md:space-x-6">
          <div className="space-y-6 flex flex-col justify-center md:w-1/2 order-2 md:order-1">
            ...
            <p className="text-lg text-neutral-300 md:max-w-lg">
          ...
          <div className="flex flex-col gap-6 lg:grid lg:grid-cols-4">
            {NEWS.map((item) => (
              <Link
                key={item.id}
                href={item.href}
                className="flex space-x-2 p-6 bg-neutral-800 border border-neutral-700 rounded-3xl lg:col-auto transition-colors hover:bg-neutral-700"
              >
                <Image
